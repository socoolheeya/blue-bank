...
val balanceServiceScenarios by testSuite("Balance rules") {
    Scenario("balance lookup returns state and missing lookup fails", ::BalanceScenarioContext) {
    ...
    Scenario("deposit and withdraw forward all arguments and update balance", ::BalanceScenarioContext) {
    ...
    listOf(BigDecimal.ZERO, BigDecimal("-1")).forEach { amount ->
        Scenario("transfer rejects $amount", ::BalanceScenarioContext) {
            Given("source and destination balances") {
                fake.balance(1, "100")
                fake.balance(2, "0")
            }
    ...
    Scenario("transfer rejects the same account", ::BalanceScenarioContext) {
        Given("an account has a balance") {
            fake.balance(1, "100")
        }
