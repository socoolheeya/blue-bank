...
val accountServiceScenarios by testSuite("Account lifecycle") {
    testFixture { AccountScenarioContext() } asContextForEach {
        ...
        limits.forEach { (product, expected) ->
            Scenario("create uses the $product default limit") {
        ...
        Scenario("missing lookups fail with the requested identifier") {
        ...
        Scenario("customer listing and modification reflect fake state") {
        ...
        Scenario("missing modification fails") {
        ...
        Scenario("close freeze and activate mutate account state") {
